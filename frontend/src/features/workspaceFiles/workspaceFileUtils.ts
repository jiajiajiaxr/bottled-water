...
export function displayNodeName(node: WorkspaceFileNode) {
  ...
  const childTitle = stripExtension(childFile?.display_name ?? childFile?.name ?? "");

  if (!isUuid(segment) && !isShortId(segment) && !isUuid(current)) {
    return normalizeKnownSegment(current);
  }
  if (path.startsWith("artifacts/")) {
    return `产物：${childTitle || "未命名产物"} · ${segment.slice(0, 8)}`;
  }
  if (path.startsWith("uploads/legacy/") || path.startsWith("files/legacy/")) {
    return `上传记录：${childTitle || segment.slice(0, 8)}`;
  }
  if (isCurrentRoleSegment(path, "tasks")) return `任务输出 ${segment.slice(0, 8)}`;
  if (isCurrentRoleSegment(path, "agents")) return `Agent 输出 ${segment.slice(0, 8)}`;
