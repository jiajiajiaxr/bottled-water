...
def _styles(sample, style_factory, colors, font_name: str, align_center, align_left) -> dict[str, Any]:
    ...
    return {
        "base": base,
        "title": style_factory("AgentHubTitle", parent=base, fontSize=27, leading=34, alignment=align_center, spaceAfter=16),
        "subtitle": style_factory("AgentHubSubtitle", parent=base, fontSize=13, leading=20, alignment=align_center),
        "h1": style_factory("AgentHubH1", parent=base, fontSize=20, leading=26, spaceBefore=12, spaceAfter=8),
        "h2": style_factory("AgentHubH2", parent=base, fontSize=15, leading=21, spaceBefore=10, spaceAfter=6),
        "h3": style_factory("AgentHubH3", parent=base, fontSize=12.5, leading=18, spaceBefore=8, spaceAfter=4),
        "quote": style_factory("AgentHubQuote", parent=base, leftIndent=10, textColor=colors.HexColor("#475569")),
        "small": style_factory("AgentHubSmall", parent=base, fontSize=8.5, leading=13, textColor=colors.HexColor("#64748b")),
    }
...
def _story(
    ...
    story = _cover(model, styles, paragraph, spacer, page_break)
...
def _cover(model: dict[str, Any], styles, paragraph, spacer, page_break) -> list[Any]:
    ...
    story.extend([spacer(1, 22), *[paragraph(_escape(item), styles["subtitle"]) for item in meta if item], page_break()])
    return story
...
def _toc(model: dict[str, Any], styles, paragraph, page_break) -> list[Any]:
    ...
    for index, section_title in enumerate(section_titles, start=1):
        flowables.append(paragraph(_escape(f"{index}. {section_title}"), styles["base"]))
...
def _table_flowable(block: dict[str, Any], available_width: float, paragraph, styles, table, table_style, colors) -> list[Any]:
    ...
    table_data = [[paragraph(_escape(cell), styles["base"]) for cell in row + [""] * (column_count - len(row))] for row in data]
