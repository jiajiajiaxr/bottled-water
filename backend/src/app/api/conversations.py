...
def _accessible_workspace(db: Session, user: User, workspace_id: str | None) -> Workspace | None:
    ...
    workspace = db.scalar(select(Workspace).where(Workspace.id == workspace_id, Workspace.deleted_at.is_(None), (Workspace.owner_id == user.id) | (Workspace.id.in_(member_ids))))
    if not workspace:
...
def _list(db: Session, user: User, workspace_id: str | None = None) -> list[dict]:
    ...
    conversations = db.scalars(_conversation_query(user.id).order_by(Conversation.is_pinned.desc(), Conversation.last_message_at.desc().nullslast(), Conversation.updated_at.desc())).all()
    if workspace_id:
...
def _get(db: Session, user: User, conversation_id: str) -> Conversation:
    conv = db.scalar(_conversation_query(user.id).where(Conversation.id == conversation_id))
    if not conv:
...
async def list_conversations(workspace_id: str | None = None, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def create_conversation(payload: CreateConversationRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def get_conversation(conversation_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def get_conversation_workflow(conversation_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
...
async def update_conversation_workflow(conversation_id: str, payload: WorkflowUpdatePayload, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
...
async def generate_conversation_workflow(conversation_id: str, payload: WorkflowGeneratePayload, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
    _ensure_can_manage(conversation, user)
    fallback = _fallback_workflow(conversation)
...
async def list_workflow_runs(conversation_id: str, latest: bool = False, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def start_workflow_run(conversation_id: str, payload: WorkflowRunStartPayload, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    _ensure_workflow_tables(db)
    conversation = _get(db, user, conversation_id)
    ...
    write_audit_log(db, user=user, action="workflow.run.start", target_type="conversation", target_id=conversation.id, detail={"run_id": run.id, "mode": run.mode}, request=None, risk_score=0.15)
    db.commit()
    db.refresh(run)
...
async def update_workflow_node_state(conversation_id: str, run_id: str, node_id: str, payload: WorkflowNodeUpdatePayload, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    _ensure_workflow_tables(db)
    conversation = _get(db, user, conversation_id)
    ...
    _sync_workflow_runtime(conversation, run)
    write_audit_log(db, user=user, action="workflow.node.update", target_type="workflow_run", target_id=run.id, detail={"node_id": node_id, "status": payload_dict.get("status"), "progress": run.progress}, request=None, risk_score=0.1)
    db.commit()
    db.refresh(run)
...
async def update_conversation(conversation_id: str, payload: UpdateConversationRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def mark_read(conversation_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
...
async def list_participants(conversation_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
...
async def add_participants(conversation_id: str, payload: AddParticipantRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
    _ensure_can_manage(conversation, user)
...
async def update_participant_role(conversation_id: str, participant_id: str, payload: ParticipantRoleUpdatePayload, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
    _ensure_can_manage(conversation, user)
...
async def remove_participant(conversation_id: str, participant_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
    _ensure_can_manage(conversation, user)
...
async def invite_participants(conversation_id: str, payload: InviteParticipantRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
    _ensure_can_manage(conversation, user)
...
async def compat_list_conversations(db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    return {"items": _list(db, user)}


@compat_router.post("/conversations", response_model=dict)
async def compat_create_conversation(payload: CreateConversationRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
...
async def compat_list_participants(conversation_id: str, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
...
async def compat_add_participants(conversation_id: str, payload: AddParticipantRequest, db: Session = Depends(get_db), user: User = Depends(get_current_user)):
    conversation = _get(db, user, conversation_id)
    _ensure_can_manage(conversation, user)
    existing = {it.agent_id for it in _active_participants(conversation) if it.agent_id}
    agents = db.scalars(select(Agent).where(Agent.id.in_([aid for aid in payload.agent_ids if aid not in existing]))).all()
    for agent in agents:
        db.add(ConversationParticipant(conversation_id=conversation.id, participant_type="agent", agent_id=agent.id, role=payload.role))
    db.commit()
    db.expire_all()
