...
- `/api/v1/skills/{id}/test` 已迁移到统一 `SkillRuntime`，手动测试会按 manifest runtime 执行并写入 `SkillRun`；`script_skill` API 测试覆盖 `file.write` + `sandbox.run` 工具链。
