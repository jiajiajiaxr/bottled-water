...
- **AI Coding Agent（Codex）**：负责阅读文档和代码、拆解任务、实现功能、定位 bug、重构模块、补充文档、整理协作资产。
- **Browser / Computer Use 自审工具**：负责像真实用户一样打开页面、点击按钮、上传文件、切换会话、检查白屏/错位/无反馈等体验问题。
- **Git 历史与文档**：负责沉淀过程证据，形成可追溯的工程交付链。

协作的核心不是“让 AI 替代开发者”，而是让 Codex 成为工程负责人身边的高强度执行伙伴：它可以快速读代码、写代码、跑验证、总结上下文；Browser / Computer Use 则负责把代码结果拉回真实用户操作场景。最终方向、质量标准和产品判断仍由人类负责人控制。
...
这里的 Skills 不是 AgentHub 平台内部的业务 Skill，而是我使用 Codex、Computer Use、Browser 和 Git 完成 AI 编程协作时沉淀出的工程技能。
```
...
- `04-skills-and-prompts.md`：AI 编程协作 Skills / Prompts 沉淀，重点记录 Codex 仓库理解、架构约束、真实产物验收、Computer Use 自审、Browser 复测和 Git 证据链。
- `05-artifact-index.md`：产物地址索引。

这个阶段的协作价值是：将“使用 AI 的事实”转化为“可说明、可复用、可展示的协作方法论”。评审看到的不是一组零散提示词，而是一套围绕 Codex 编程、桌面/浏览器自审、Git 留痕和真实产物验收形成的工程协作资产。
...
### 7.1 Computer Use / Browser 自审机制

在本项目中，AI 协作不止发生在代码编辑器里，还延伸到真实界面操作。项目负责人多次通过截图指出“看起来完成但实际不可用”的问题，例如：产物预览空白、工作流画布输入框回弹、文件树错位、会话切换后流式消息消失、部署 URL 打开空白等。

为了解决这类问题，协作过程中逐步形成了 Computer Use / Browser 自审机制：

1. **真实页面打开**：不是只看代码，而是启动前后端并打开本地页面。
2. **真实用户操作**：执行发送消息、点击预览、上传文件、切换会话、打开工作区文件、运行工作流等动作。
3. **视觉和交互检查**：观察白屏、错位、按钮无反馈、弹窗空内容、状态残留。
4. **链路反推**：从 UI 现象反查前端事件、API 返回、WebSocket/SSE 事件、数据库记录和后端服务。
5. **复测闭环**：修复后回到同一场景再次操作，而不是只依赖静态代码判断。

这部分能力是本项目 AI 协作资产中很重要的一环：它证明 AI 不只是生成代码，也能在人的引导下进入“自我审查、自我复现、自我修复”的工程闭环。
